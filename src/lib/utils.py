...
def getListOfFiles(dirName, ext, path_pattern='', allFiles=list()):
    ...
    for entry in listOfFile:
        ...
        if os.path.isdir(fullPath):
            allFiles = allFiles + \
                getListOfFiles(fullPath, ext=ext, path_pattern=path_pattern)
        else:
    ...
    for entry in listOfFolder:
        ...
        if os.path.isdir(fullPath):
            allFolders = allFolders + \
                getListOfFolders(fullPath, ext=ext, path_pattern=path_pattern)
        else:
...
def display_detection(img, img_box, matching_score, mask,  msg='', cmap="cividis"):
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(
        1, 3, sharex=True, sharey=True, figsize=(30, 8))
    ...
    for entry in listOfFile:
        ...
        if os.path.isdir(fullPath):
            ...
            if extension == ext:
                allFiles.append(fullPath)
        # print(f'\n dirName[{ext}]= {dirName}\n allFiles={allFiles}')
    allFiles = [k for k in allFiles if ext in os.path.splitext(k)[
        1][1:] == ext]
    return list(set(allFiles))
...
def get_box_coordinate(image_path, detection_boxes, w, h, defect_class, xml_list=[], disp=0):
    ...
    if disp >= 1:
        print("\n\n\n ######\n Image = %s \ndetection = %s" %
              (image_path, detection_boxes))
    # get file ID / prefix
    file_prefix, file_extension = os.path.splitext(
    ...
    for pascal_voc_box, defect_name in zip(detection_boxes, defect_class):
        ...
        yolo_box = pascal_voc_to_yolo(
            pascal_voc_box[0], pascal_voc_box[1], pascal_voc_box[2], pascal_voc_box[3], w, h)
        column_name = ['filename', 'image_id',
...
def save_xml_mask_annotations(img, mask, data_tag, annotation_directory,  label_boxes, img_class, with_masks=True,
    ...
    if with_masks == False:
        ...
        annotation_directory = os.path.join(
            annotation_directory, img_class, os.path.dirname(data_tag))
        ...
        image_path = os.path.join(
            annotation_directory, 'images', data_tag + img_ext)
        save_array_to_2rgb(img, image_path)
        mask_path = os.path.join(annotation_directory,
                                 'masks', data_tag + mask_ext)
        ...
        # Assign the default name of defectes as  CLASS_MAPPING[1]->'defective'
        if defect_label == '':
            defect_label = [CLASS_MAPPING[1] for defect in label_boxes]
        xml_list = get_box_coordinate(
            image_path, label_boxes, w, h, defect_label, xml_list=xml_list)
        # Save  all annotation in csv file
        column_name = ['filename', 'image_id',
                       'width', 'height', 'bbox', 'inspection']
        ...
        DIR_CLASS_info = os.path.join(
            os.path.dirname(CSV_DATA_FILE), 'info.json')
        Class_dict = {"source": 'generated  using Hybrid inspection ', "class_names": {
            str(k): name for k, name in enumerate(CLASS_MAPPING)}}
...
def create_object_boxes(img_inpt0, mask, Min_box_area=0, factor=1.2, with_masks=True, data_tag='', annotation_directory='', disp=1):
    ...
    if img_inpt.max() <= 0:  # image is black/zeros
        ...
        img_box = img_0.copy()
        img_box = 250*(img_box/img_box.max())
        if disp >= 3:
            display_detection(img_inpt, img_box, mask,
                              msg=' input to create boxes', cmap="gray")
        try:
        ...
        if disp >= 3:
            display_detection(
                img_inpt, img_box, mask, msg=' input to create boxes [gray scale]', cmap="gray")
        if np.max(img_box) != 0:
            ...
            if len(props) < mask_area/2:
                for prop in props:
                    box_obj = [prop.bbox[1], prop.bbox[0],
                               prop.bbox[3], prop.bbox[2]]
                    box_area = (box_obj[2]-box_obj[0]) * \
                        (box_obj[3]-box_obj[1])
                    if disp >= 3:
                        print(f'\nprop={prop}')
                    ...
                    if box_area > Min_box_area:
                        ...
                        nb_box += 1
                        # update th  mask
                        mask_out[prop.bbox[0]:prop.bbox[2], prop.bbox[1]:prop.bbox[3]
                                 ] = mask[prop.bbox[0]:prop.bbox[2], prop.bbox[1]:prop.bbox[3]]
                        if disp >= 3:
                            display_detection(
                                img_inpt, img_box, mask_out, msg='Bouding boxes', cmap="gray")
            msg_org = (int(0.1*mask.shape[1]), int(0.95*mask.shape[0]))
            if nb_box == 0:
                # print('\n\n\n Warning: No fault is detected !!!!')
                img_box = cv2.putText(img=np.copy(img_box), text="Good road!", org=msg_org,
                                      fontFace=1, fontScale=1, color=(0, 255, 0), thickness=1)
                img_class = 'defect-free'
            else:
                img_box = cv2.putText(img=np.copy(img_box), text=str(
                    nb_box) + " damage detected!", org=msg_org, fontFace=1, fontScale=1, color=(255, 0, 0), thickness=1)
                img_class = 'defective'
            ...
            print(
                f'boxes creation is ignored as the <img_box> is black (max value= {np.max(img_box)} )')
            img_class = 'defect-free'
            return img_inpt, mask_out, nb_box, label_boxes, img_class
...
def plot_cv_bbox(img, voc_box):
    ...
    cv2.rectangle(img_box, (voc_box[1], voc_box[0]),
                  (voc_box[3], voc_box[2]), (255, 0, 0), 4)
...
def background_removel(img, bs_level=1, disp=1):
    ...
    if bs_level == 1:
        ...
        _, binary = cv2.threshold(
            img_gray, 0.5*img_gray.max(), img_gray.max(), cv2.THRESH_BINARY_INV)
...
def search_similar_segment(input_mean, input_std, mean_, std_, mean_th=0.2, std_th=0.2):
    ...
    for cnt, (m, s) in enumerate(zip(mean_, std_)):
        if m*(1-mean_th) <= input_mean <= m*(1+mean_th):
            # input_std**2 <= (s**2)*(1+std_th) :# (s**2)*(1-std_th) <=
            if std_th == 0.0 or input_std <= std_th*s:
...
def refine_segment_statistic(segments, img, mean_th=0.2, std_th=0.2, nb_defect_segment=1, mean_=[], std_=[]):
    ...
    for seg in list_segments:
        ...
        seg_new, mean_, std_ = search_similar_segment(
            mean, st, mean_, std_, mean_th, std_th)
...
def segment_image_DSP(img0, n_segments=1000, compactness=10, mean_th=0.3, std_th=0.9, nb_defect_segment=3, disp=1):
    ...
    segments_slic_1 = slic(img, n_segments=n_segments,
                           compactness=compactness, sigma=0.5, start_label=1)
    ...
    if disp >= 1:
        ...
        ax[0, 0].set_title(
            f'SLIC segment Boundaries [{len(np.unique(segments_slic_1))} segments]')
        ax[1, 0].imshow(img)  # mark_boundaries(img, segments_slic_refined))
        ax[1, 0].set_title('Original image')
        ax[0, 1].imshow(segments_slic_refined, cmap='hot')
        ax[0, 1].set_title(
            f'proposed SLIC segments refining [{len(np.unique(segments_slic_refined))} segments]')
        ...
        ax[1, 1].imshow(detected_defect, cmap='Reds',
                        interpolation='none', alpha=0.4)
        ax[1, 1].set_title(
            f'Defect detection [defect_th = {nb_defect_segment} segments] ')
