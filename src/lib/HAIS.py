...
from lib import hais_database, inspection_algorithm, inspection_map, utils
...
class PreferencesForm(QDialog):
    def __init__(self, preferences_path):
        ...
        # preferences_path="bin/preferences.json"
        self.preferences_path = preferences_path
        self.default_path = os.path.join(os.path.dirname(
            preferences_path), "preferences_default.json")
        ...
        buttonBox.button(QDialogButtonBox.Yes).clicked.connect(
            self.save_apply_data)
...
class HAIS_GUI(QWidget):
    def __init__(self, config_file, disp=0):
        ...
        self.root_folder = os.path.join(
            workspace, self.config['DATA']['data_root'])
        ...
        self.img_size = (
            self.config['INSPECTION']['imgSizes_X'], self.config['INSPECTION']['imgSizes_Y'])
        self.viewer_resize = (1200, 800)
        self.DB = None
        self.dst_directory = os.path.join(
            workspace, self.config['OUTPUTS']['dst_directory'])
        self.maps_root = os.path.join(self.dst_directory, "maps")
        self.annotation_directory = os.path.join(
            workspace, self.config['ANNOTATION']['annotation_directory'])
    ...
    def init_GUI(self):
        ...
        self.data_source.stateChanged.connect(
            lambda: self.btnstate(self.data_source))
        self.input_data_button = QPushButton('Click')  # browse file
        self.input_data_button.clicked.connect(self.get_image_path)

        # Run data inspection
        self.frame_step = 1
        self.button_previous_frame_inspection = QPushButton(
            'inspect previous frame ')
        self.button_previous_frame_inspection.setFont(
            QFont("Times", 10, QFont.Bold))
        self.button_previous_frame_inspection.clicked.connect(
            self.inspect_previous_frame)

        self.button_next_frame_inspection = QPushButton('inspect next frame ')
        self.button_next_frame_inspection.setFont(
            QFont("Times", 10, QFont.Bold))
        self.button_next_frame_inspection.clicked.connect(
            self.inspect_next_frame)
        ...
        # TODO flag:  integrate the napari verification ( pip  install napari[pyqt5] )
    ...
    def display(self):
        ...
        try:
            print(
                f'## - Automated annotation destination root : {self.annotation_directory} ')
        except:
            err = 1
        print(f'################################################################################\n\n')
    ...
    def on_trip_selection(self):
        ...
        # get list of sensors
        self.DB = hais_database.HAIS_database(
            dataroot=self.dataroot, version=self.version, verbose=self.verbose)
        if self.DB.err == 1:
            ...
            self.out_metric_arr = np.array(
                [0 for k in range(len(self.inspection_dict['token']))])
            # print(f'\n  inspection_dict = {self.DB.inspection_dict}')

            # list sensors names
            # list_sensors=self.DB.get_list_sensors()
            sensors_file = os.path.join(
                self.dataroot, self.version, 'sensor.json')
            list_sensors = utils.load_json(sensors_file)
            # print(f'\n flag: \n - list_sensors0={list_sensors}')
            list_sensors = [
                k["channel"] for k in list_sensors if 'CAM' in k["channel"] or 'LIDAR' in k["channel"]]
    ...
    def on_sensor_selection(self):
        ...
        if self.node_name == 'select a sensor':
            return
        self.sensor_folder = os.path.join(
            self.dataroot, 'sweeps', self.sensor_name)
        # retreive list of files
        self.list_files = [path for path in glob(os.path.join(
            self.sensor_folder, '*')) if os.path.isfile(path)]
    ...
    def show_inspection_map(self):
        ...
        if self.show_all_nodes.isChecked():  # show all nodes
            ...
            list_trips = [self.trip_dict[trip_name]
                          for trip_name in self.trip_dict.keys()]
    ...
    def visualize_image(self, input_data):
        ...
        try:
            ...
            QMessageBox.warning(self, "Image Viewer",
                                "Cannot view the data %s." % filename)
            print(f'Exception; {e}')
            return
    ...
    def create_visualization_formGroupBox(self):
        ...
        # TODO flag: enable forcasting algorithm/button
    ...
    def browse_data_path(self):
        ...
        if self.data_source.isChecked() == True:
            data_path = QFileDialog.getOpenFileName(
                self, f'Select a trip file ', self.root_folder, "Images (*.png *.jpeg *.jpg *.bmp )")[0]
        else:
            data_path = QFileDialog.getExistingDirectory(
                self, 'Select the trip folder ', self.root_folder)
    ...
    def define_nodes_and_trips(self, root):
        camera_folders = []
        camera_folders = utils.getListOfFolders(
            root, ext='.jpg', path_pattern='sweeps')
    ...
    def explore_nodes_and_trips(self, root):
        ...
        idx_list = [i for i in range(self.list_sensors_combo.count(
        )) if 'CAM' in self.list_sensors_combo.itemText(i)]
    ...
    def define_module_from_sensor_name(self):
        # # TODO flag : define the detection algorithm: lanemarker or road condition algorithms
    ...
    def stop_thread(self):
        ...
        self.button_run_inspection.setStyleSheet(
    ...
    def run_data_inspection_sequences(self, n):
        ...
        if n == N-1:
            ...
            # # TODO flag: recheck the data storage for future analysis
            # ##---------------  FLAG --------------------
            ...
            self.button_run_inspection.setText(
                'Click to stop the inspection sequence ...')
        ...
        for k, sensor_name in enumerate(self.sensor_data_dict.keys()):
            ...
            if sensor_name == self.sensor_name:
                if sensor_name == 'RIGHT_CAMERA' or sensor_name == 'LEFT_CAMERA':
                    ...
                    reflection_coef, lanemarker_img, image_RGB, mask = inspection_algorithm.lane_inspection(
                        img_path, disp=0)  # , bright_th=bright_th)
                    # get diagnosis
                    out_metric, color = inspection_algorithm.get_lanemarker_diagnosis(
                        reflection_coef)
                    ...
                    if self.sensor_name == sensor_name:
                        msg = f"progress:{progress}\t fileID: " + \
                    ...
                    out_metric, color = inspection_algorithm.get_road_diagnosis(
                        nb_damages, dict_damage)
                    out_image = inspection_algorithm.write_damage_report_on_image(
                        out_image, dict_damage, nb_damages, color)
    ...
    def save_labeling_image(self, class_folder):
        if os.path.exists(self.image_path):
            ...
            self.image_ID.setText(msg)
            import shutil
            dst_file = os.path.join(self.annotation_directory, 'road_quality',
                                    class_folder, os.path.basename(self.image_path))
    ...
    def run_VGA_annotator(self):
        ...
        for file in ['../webapp/templates/via-annotator.html', 'webapp/templates/via-annotator.html']:
    ...
    def save_image_and_mask(self, node_name, trip_name, sensor_name, image_id, image, mask):
        if len(np.unique(mask)) > 1:  # ignore empty or full masks
            ...
            dst_folder = os.path.join(
                self.annotation_directory, node_name, f'{trip_name}__{sensor_name}')
            # save the image
...
def get_tool_scan_names_path(mask_path):
    # os.path.basename(os.path.dirname(os.path.dirname(self.mask_path)))
    tool_name = get_folder_tag(get_n_parents_folders_from_path(
        os.path.dirname(mask_path), TAG='', n=2))
    scan_name = os.path.basename(os.path.dirname(mask_path))
    return tool_name, scan_name


def get_folder_tag(path):
    TAG = os.path.basename(os.path.dirname(os.path.dirname(
