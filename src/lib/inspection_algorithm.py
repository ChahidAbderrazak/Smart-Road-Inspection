...
def plot_dsp_detection(road_hole, image, matching_score, mask, detection_list):
    ...
    for x, y, wcoin, hcoin in detection_list:
        rect = plt.Rectangle((x, y), wcoin, hcoin,
    ...
    # highlight matched region
    # ax4.autoscale(False)
    ax4.plot(x, y, '*', markeredgecolor='r',
             markerfacecolor='none', linewidth=5, markersize=30)
...
def detect_road_damage_using_patch_matching(image_RGB, road_hole_RGB, disp=False):
    ...
    mask = np.pad(result, [((wcoin-1)//2, (hcoin-1)//2),
                  ((wcoin-1)//2, (hcoin-1)//2)], mode='constant')
...
def inspection_patch_matching(img_path, hole_patch_root, patch_size=(256, 256), detect_th=0.75, disp=True):
    ...
    for patch_path in patch_paths:
        ...
        if len(np.unique(road_hole_RGB)) == 1:
            continue
        # run the detection
        mask_, result_, detection = detect_road_damage_using_patch_matching(
            image_RGB, road_hole_RGB)
    ...
    # plot the dsp detection results
    if disp:
        plot_dsp_detection(road_hole_RGB, image,
                           matching_score, mask, detection_list)
...
def auto_thresholding(mask0, disp=1):
    ...
    if disp >= 1:  # True: # flag
        ...
        plt.vlines(idx, plt.ylim()[0], plt.ylim()[1],
                   linestyles='dashed', label='Auto-Th')
        plt.title(
            f'idx0={idx0}. idx_max={idx_max} idx={idx}, auto_th={bin[idx]} [{cnt} loops]')
        plt.show()
    return auto_th
...
def residual_thresholding(image0, diff_image, bright_th=160, erosion_tol=1,
    ...
    if disp:
        utils.show_two_image(diff_image, image0,
                             img_title=f'diff image', figsize=(8, 8))
    ...
    # display
    if disp:
        utils.show_two_image(
            image, mask, img_title=f'road damages', figsize=(8, 8))
...
def segmenting_road(img, disp=False):
    ...
    labels1 = segmentation.slic(
        img, compactness=10, n_segments=2000, start_label=1)
...
def image_erosion(img, erosion_tol=5):
    ...
    from scipy import ndimage
    img_filter = ndimage.binary_erosion(
        img_gray, structure=np.ones((erosion_tol, erosion_tol))).astype(float)
    for k in range(3):
...
def lane_inspection(img_path, bright_th=160, erosion_tol=1, disp=1):
    ...
    # image thresholding
    lane, lane_mrk_mask, nb_cnts = image_thresholding(
        image=image, bright_th=bright_th, erosion_tol=erosion_tol)
    ...
    if disp > 0:
        utils.show_two_image(
            image, lane, img_title=f'lane mark image [reflection coefficient ={reflection_coef:.2f}]', figsize=(8, 8))
...
def main_DSP_segmentation():
    ...
    # inspect the images
    out = cv2.VideoWriter(
        out_video, cv2.VideoWriter_fourcc(*'MJPG'), 10, img_size)
    list_images = utils.getListOfFiles(
        dirName=img_folder, ext=img_ext, path_pattern='')
    list_images.sort()
...
def main_patch_matching_inspection():
    ...
    for img_path in list_images[5:]:  # :#
        ...
        img, img_rgb, matching_score, mask = inspection_patch_matching(
            img_path, hole_patch_root, detect_th=detect_th, disp=False)

        # creat ebouding boxes
        img_box, mask_out, nb_box, label_boxes, img_class = utils.create_object_boxes(
            img, mask, Min_box_area=Min_box_area, factor=factor, disp=0)
    ...
    if nb_damages > 1:
        ...
        for i, damage in enumerate([f'Damaged report [{nb_damages} damages]:'] + list(dict_damage.keys())):
            ...
            cv2.putText(damage_img, f'{damage_str}',
                        (int(0.01*n), int(0.1*m) + 30 *
                         cnt), cv2.FONT_HERSHEY_SIMPLEX,
                        0.6, color, 2)
            cnt += 1
    return damage_img
...
def main_image_variation_inspection():
    ...
    for k, img_path in enumerate(list_images):  # [-5:]):#
        ...
        damage_img = write_damage_report_on_image(
            damage_img, dict_damage, nb_damages, color)
...
def main_lanemarker_inspection():
    ...
    out = cv2.VideoWriter(
        out_video, cv2.VideoWriter_fourcc(*'MJPG'), 10, img_size)
    # enumerate([1:2]):#
    for k, img_path in enumerate(glob(os.path.join(img_folder, '*'))):
        # DSP-based road segmentation
        reflection_coef, lanemarker_img, image, lane_mrk_mask = lane_inspection(
