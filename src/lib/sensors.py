...
colors_list = ['#0051a2', '#97964a', '#ffd44f', '#f4777f', '#93003a',
...
class RPLidar_sim(object):
    ...
    def plot_lidar(self, num_scans=100):
        ...
        lidar_img = np.array([xi+[0]*(length-len(xi))
                             for xi in join_distance_list])
...
class HAIS_visualizer(RPLidar_sensor):
    ...
    def visualize(self):
        ...
        for k,	scan in enumerate(tqdm(scans_dict_list)):
            ...
            if scan != {}:
                ...
                # Kinemetics
                    AccXangle, AccYangle = new_[
                        'ACCX Angle'], new_['ACCY Angle']
                    gyroXangle, gyroYangle, gyroZangle = new_[
                        'GRYX Angle'], new_['GYRY Angle'], new_['GYRZ Angle']
                    # new_['CFangleY Angle'],
                    CFangleX, CFangleY = new_['CFangleX Angle'], 0
                    heading, tiltCompensatedHeading = new_[
                        'HEADING'], new_['tiltCompensatedHeading']
                    kalmanX, kalmanY = new_['kalmanX'], new_['kalmanY']
                    Kinematics_list.append([AccXangle, AccYangle, gyroXangle, gyroYangle, gyroZangle,
                                           CFangleX, CFangleY, heading, tiltCompensatedHeading, kalmanX, kalmanY])
                    if self.disp:
    ...
    def plot_Kinemetics(self, Kinematics_list):
        ...
        AccXangle, AccYangle, gyroXangle, gyroYangle, gyroZangle, CFangleX, CFangleY, heading, tiltCompensatedHeading, kalmanX, kalmanY =\
            arr[:, 0], arr[:, 1], arr[:, 2], arr[:, 3], arr[:, 4], arr[:,
        ...
        list_metrics = [diff(AccXangle), diff(AccYangle), diff(gyroXangle), diff(gyroYangle), diff(gyroZangle), diff(
            CFangleX), diff(CFangleY), diff(heading), diff(tiltCompensatedHeading), diff(kalmanX), diff(kalmanY)]
        list_label = ['AccXangle', 'AccYangle', 'gyroXangle', 'gyroYangle', 'gyroZangle',
                      'CFangleX', 'CFangleY', 'heading', 'tiltCompensatedHeading', 'kalmanX', 'kalmanY']
        linestyles = ["-", "-", "--", "--",
                      "--", "-.", "-.", "-", "-", ":", ":"]
        linewidth_list = [2.0, 2.0, 1.5, 1.5, 1.5,
                          1.5, 1.2, 1.2, 1.0, 1.0, 0.8, 0.8]
        for var, label, color, linestyle, linewidth in zip(list_metrics, list_label, colors_list[:len(list_metrics)], linestyles, linewidth_list):
            line, = ax3.plot(X, var, linestyle=linestyle,
...
def run_Lidar_sim():
    ...
    IMIN = 0
    IMAX = 50
    # TODO flag: check this testing
    filename = 'data/test/catalog_0_A1_R5_465_1.catalog.txt'
    lidar = RPLidar_sim(filename, angle_step=2,
                        DMAX=DMAX, IMIN=IMIN, IMAX=IMAX)
