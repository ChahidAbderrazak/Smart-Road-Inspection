...
class HAIS_map(object):
    ...
    def __init__(self, place_coord, radius=1000, max_St=20):
        ...
        self.route_list = []
        self.route_colors = []
        self.available_colors = ['red', 'salmon',
                                 'orange', 'yellow', 'green', 'green']
        # maximum streets annotation to visualize (eg: -1 for all)
        self.max_St = max_St
    ...
    def initialize_location_map(self):
        ...
        # , network_type="drive")
        self.graph = ox.graph_from_point(self.place_coord, dist=self.radius)
        self.origin = ox.nearest_nodes(
            self.graph, X=self.place_coord[0], Y=self.place_coord[1])
        # Retrieve nodes and edges
        self.nodes, self.edges = ox.graph_to_gdfs(self.graph)
        # self.list_noes_coord()
        # get building
        self.buildings = ox.geometries_from_point(self.place_coord, dist=self.radius, tags={
                                                  'building': True})  # Retrieve buildings from the area:
        # print(f'\n buildings.columns={buildings.columns}')
        # Retrieve parks
        self.leisure = ox.geometries_from_point(
            self.place_coord, dist=self.radius, tags={'leisure': True})  # fetch data
        self.parks = self.leisure[self.leisure["leisure"].isin(
            ["pitch", "park", "playground"])]  # select all park polygons
        # Get water bodies map of the New York City area
        self.water = ox.geometries_from_point(
            self.place_coord, dist=self.radius, tags={"natural": "water"})

    def list_noes_coord(self):
        list_coord = []
        for node_id in list(self.graph.nodes):
            list_coord.append([self.graph.nodes[node_id]['x'],
    ...
    def get_random_point(self):
        ...
        node_ = self.graph.nodes[orig]
        start_point = (node_['x'], node_['y'])
        node_ = self.graph.nodes[dest]
        dest_point = (node_['x'], node_['y'])
    ...
    def generate_random_road_slices(self, Nroutes=5):
        ...
        for k in range(Nroutes):
            ...
            start = ox.nearest_nodes(
                self.graph, X=start_point[0], Y=start_point[1])
            dest = ox.nearest_nodes(
                self.graph, X=dest_point[0], Y=dest_point[1])
            if start != dest:
    ...
    def convert_coord_to_nodes(self, list_inspected_coord):
        ...
        for k in range(len(list_inspected_coord)-1):
            ...
            start = ox.nearest_nodes(
                self.graph, X=start_point[1], Y=start_point[0])
            dest = ox.nearest_nodes(
                self.graph, X=dest_point[1], Y=dest_point[0])

            print(f'\n start={start}, dest={dest}')
            if start != dest:
                list_coordinate.append(
                    (start, dest, int(np.mean(road_metric))))
                new_portion = True
            else:
                new_portion = False
    ...
    def get_roads_conditions(self, list_inspected_roads):
        ...
        for orig, dest, road_metric in list_inspected_roads:
            ...
            if len(route) > 0:
                ...
                # [route_color]*(len(route) - 1))
                self.route_colors.append(route_color)
...
def generate_random_roads_inspection():
    ...
    place_coord = (43.94593458427975, -78.89566960105887)
    radius = 1000  # meters
    max_St = -1
...
def visualize_map(list_missions, maps_root, horison=2, show_lanemarker=False, show_all_nodes=False):
    ...
    # ['gray', 'red', 'coral', 'orange', 'aquamarine2', 'green']
    available_colors = ['gray', 'red',  'orange',  'green', 'green']
    if show_lanemarker:
        inspect_status = ['Scanned lanemarkers', 'Low reflectivity',
                          'Medium reflectivity', 'Good reflectivity', 'Excellent reflectivity']
    else:
        inspect_status = ['Scanned road', 'Bad road',
                          'Regular road', 'Good road', 'Excellent road']
    # load HAIS data
    ...
    print(
        f' - The map will show {len(list_missions)} missions. Please wait :) ...\n - missions={list_missions}')
    for dataroot in list_missions:
        list_files = utils.getListOfFiles(
            dataroot, ext='.json', path_pattern='inspection_dic.json')
        ...
        for inspect_filename in list_files:
            ...
            if not os.path.exists(inspect_filename):
                ...
                except:  # TODO flag: I am not sure why I made this try/catch!!!!
    ...
    if show_all_nodes:
        ...
        map_path = os.path.join(
            maps_root, 'inspection_Map_' + os.path.basename(dataroot)+'.html')
    ...
    draw_polylines(points, metrics, myMap, available_colors,
                   inspect_status, horison=horison)
