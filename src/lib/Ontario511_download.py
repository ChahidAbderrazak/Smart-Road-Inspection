        ...
        if not corrp_img:
            ...
            with open(filePath, 'wb') as localFile:
                ...
                camera_info = os.path.join(
                    os.path.dirname(filePath), 'camera_info.json')
                ...
                if disp:
                    ...
                    # show image
                    self.show_image(
                        img, img_title=f'{os.path.basename(filePath)} [error={corrp_img}]')
            localFile.close()
                ...
                if cam_enabled == "Enabled":
                    ...
                    if RoadwayName != None:
                        filePath = os.path.join(
                            self.dst_root, "CAMERAS", camera_dict['RoadwayName'], camera_ID, timestamp+'.jpg')
    ...
    def download_road_conditions(self, disp=False):
        ...
        for data_dict in tqdm(road_dict[0:-1]):
            try:
                ...
                datetime_object = str(
                    datetime.datetime.fromtimestamp(LastUpdated))
                ...
                if RoadwayName != None:
                    filePath = os.path.join(
                        self.dst_root, "road-conditions", road_class, RoadwayName, str(LastUpdated) + '.json')
