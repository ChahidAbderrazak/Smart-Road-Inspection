...
class PreferencesForm(QDialog):
  def __init__(self):
    ...
    self.default_path = "bin/preferences_default.json"
    self.head0 = QLabel("<h2>CTIMS Preferences</h2><br>")
    self.empty = QLabel("")

    self.head1 = QLabel("<h3>System Safety Parameters:</h3>")
    self.safe_min_thr = QLineEdit()
    self.qlab11 = "Minimum Acceptable Defect (in percent):"
  ...
  def set_form_data(self):
    cur_data = utils.load_json(self.preferences_path)
    self.safe_min_thr.setText(str(cur_data['safe_min_thr_val']))

  def create_form(self):
    print("Opening CTIMS Preferences...")
    self.formGroupBox = QGroupBox()
    layout = QFormLayout()
    layout.addRow(self.head0, QLabel(""))
    layout.addRow(self.head1, QLabel(""))
    layout.addRow(QLabel(""), QLabel(""))
    layout.addRow(QLabel(self.qlab11), self.safe_min_thr)
    layout.addRow(QLabel(""), QLabel(""))
    layout.addRow(QLabel(""), QLabel(""))
  ...
  def save_data(self):
    safe_min_thr_val = self.safe_min_thr.text()
    ret = self.set_safety_min_thr(safe_min_thr_val)
    if ret: return
    # Saving the new settings
    param_obj = {'safe_min_thr_val': int(safe_min_thr_val)}
    utils.save_json(param_obj, self.preferences_path)
    return True

  def show_info_dialog(self, message):
    msg = QMessageBox()
    msg.setWindowTitle("Preferences Accepted")
  ...
  def non_digit_error(self, val):
      try:
          val = float(val)
          return False
      except:
          msg = "Plese enter a numerical value"
          QMessageBox.warning(self, "Format error", msg )
          return True

  def set_safety_min_thr(self, val):
      ret = self.non_digit_error(val)
      if ret: return ret
...
class Menur(QWidget):
  ...
  def set_preferences(self):
    self.preferences_window = PreferencesForm()
