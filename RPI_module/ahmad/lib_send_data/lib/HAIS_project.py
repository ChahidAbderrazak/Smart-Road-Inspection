...
def commpresss(th=1000000):
    ...
    for n in nodes:
        path=os.path.join(row_data_path,n)
        os.chdir(path)
        row_data_sweep=os.path.join(path,'sweeps')
        ...
        mapname=node_name+"map_"+str(time.ctime()).replace(' ','_').replace(':','_')+'.tar'
        ...
        size=0
        c=0
        dat=[f for f in os.listdir(path) if  '.tar' not in f]
        for fi in dat:
                        data= fi
        ...
        for i in os.listdir(path):
            shutil.move(os.path.join(path,i),os.path.join(root,i))
        os.chdir(origen)
